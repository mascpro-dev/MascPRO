...
import {
  Calendar, Plus, Clock, User, Phone, Scissors, X, Check,
...
const STATUS_CONFIG: Record<string, { label: string; cor: string; icon: any }> = {
  ...
  concluido:  { label: "Conclu├¡do",  cor: "bg-green-900/30 text-green-400 border-green-800/40",    icon: CheckCircle },
  cancelado:  { label: "Cancelado",  cor: "bg-red-900/30 text-red-400 border-red-800/40",          icon: XCircle },
};

const DIAS_SEMANA = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "S├íb"];
const DIAS_COMPLETO = ["Domingo","Segunda","Ter├ºa","Quarta","Quinta","Sexta","S├íbado"];
...
export default function AgendaPage() {
  ...
  const [form, setForm] = useState({ client_name: "", client_phone: "", service: "", appointment_date: "", appointment_time: "", duration_min: "60", price: "", notes: "" });
  ...
  function abrirEditar(a: Appointment) {
    ...
    setForm({
      ...
      duration_min: String(a.duration_min), price: a.price ? String(a.price) : "", notes: a.notes || "",
    });
    setShowForm(true);
  }

  // Disponibilidade helpers
  function getDia(dow: number) {
    return disponibilidade.find(d => d.day_of_week === dow);
  }
  function toggleDia(dow: number) {
    const existe = getDia(dow);
    if (existe) {
      setDisponibilidade(prev => prev.filter(d => d.day_of_week !== dow));
    } else {
      setDisponibilidade(prev => [...prev, { day_of_week: dow, start_time: "09:00", end_time: "18:00", slot_duration_min: 60, active: true }]);
    }
  }
  ...
  async function salvarDisp() {
    ...
    await fetch("/api/agenda/disponibilidade", {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">{appointments.length} agendamento(s) {pendentes > 0 && <span className="text-yellow-400 font-bold">┬À {pendentes} pendente(s)</span>}</p>
            </div>
          </div>
        ...
        {userId && (
          <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl px-4 py-3 mb-5 flex items-center gap-3">
            ...
            <p className="text-xs text-zinc-400 flex-1 truncate font-mono">{window?.location?.origin || ""}/agendar/{userId}</p>
          ...
          <>
            ...
            {loading ? (
              ...
              <div className="text-center mt-16 text-zinc-600">
                ...
                <p className="font-bold uppercase text-sm">Nenhum agendamento neste m├¬s</p>
                  ...
                  return (
                    <div key={data}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[11px] font-black uppercase text-zinc-500 tracking-widest">{diaSemana} ┬À {formatarData(data)}</p>
                      </div>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-4 text-xs text-zinc-500 flex-wrap">
                                    ...
                                    {a.client_phone && <span className="flex items-center gap-1"><Phone size={11} /> {a.client_phone}</span>}
        ...
        {aba === "config" && (
          <div>
            <p className="text-xs text-zinc-500 mb-4">Configure os dias e hor├írios que voc├¬ atende. Seus clientes s├│ poder├úo agendar dentro desses hor├írios.</p>
                ...
                return (
                  <div key={dow} className={`bg-zinc-900/60 border rounded-2xl p-4 transition-all ${ativo ? "border-zinc-700" : "border-zinc-900 opacity-50"}`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {ativo && (
                        <span className="text-[10px] text-zinc-500">{dia.start_time?.slice(0,5)} ÔÇô {dia.end_time?.slice(0,5)}</span>
                      )}
                    </div>
                    {ativo && (
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <input type="time" value={dia.start_time?.slice(0,5) || "09:00"}
                            onChange={e => setDiaField(dow, "start_time", e.target.value)}
                            className={inputClass} />
                        </div>
                        <div>
                          <label className={labelClass}>Fim</label>
                          <input type="time" value={dia.end_time?.slice(0,5) || "18:00"}
                            onChange={e => setDiaField(dow, "end_time", e.target.value)}
                            className={inputClass} />
                        </div>
                        <div>
                          <label className={labelClass}>Dura├º├úo (min)</label>
                          <select value={dia.slot_duration_min || 60}
                            onChange={e => setDiaField(dow, "slot_duration_min", Number(e.target.value))}
                            className={inputClass}>
            ...
            <button onClick={salvarDisp} disabled={salvandoDisp}
              className="w-full mt-5 bg-[#C9A66B] hover:bg-[#b08d55] disabled:opacity-60 text-black font-black uppercase text-xs tracking-widest py-3 rounded-xl flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 sticky top-0 bg-zinc-950">
              <h2 className="font-black uppercase text-sm">{editando ? "Editar Agendamento" : "Novo Agendamento"}</h2>
              <button onClick={() => setShowForm(false)}><X size={20} className="text-zinc-500" /></button>
            </div>
            <div className="p-6 space-y-3">
              <div><label className={labelClass}>Nome do Cliente *</label><input value={form.client_name} onChange={e => set("client_name", e.target.value)} className={inputClass} placeholder="Ex: Jo├úo Silva" /></div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className={labelClass}>Hor├írio *</label><input type="time" value={form.appointment_time} onChange={e => set("appointment_time", e.target.value)} className={inputClass} /></div>
              </div>
              <div><label className={labelClass}>Servi├ºo</label><input value={form.service} onChange={e => set("service", e.target.value)} className={inputClass} placeholder="Ex: Corte + Barba" /></div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelClass}>Dura├º├úo</label>
              ...
              <div><label className={labelClass}>Observa├º├Áes</label><textarea rows={2} value={form.notes} onChange={e => set("notes", e.target.value)} className={`${inputClass} resize-none`} placeholder="Observa├º├Áes..." /></div>
              <button onClick={salvar} disabled={salvando || !form.client_name || !form.appointment_date || !form.appointment_time}
                ...
                {editando ? "Salvar Altera├º├Áes" : "Agendar"}
              </button>
            </div>
          </div>
        </div>
