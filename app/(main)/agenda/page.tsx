        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">{appointments.length} agendamento(s) {pendentes > 0 && <span className="text-yellow-400 font-bold">· {pendentes} pendente(s)</span>}</p>
            </div>
          </div>
                ...
                return (
                  <div key={dow} className={`bg-zinc-900/60 border rounded-2xl p-4 transition-all ${ativo ? "border-zinc-700" : "border-zinc-900 opacity-50"}`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {ativo && (
                        <span className="text-[10px] text-zinc-500">{dia.start_time?.slice(0,5)} – {dia.end_time?.slice(0,5)}</span>
                      )}
                    </div>
                    {ativo && (
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-3">
              ...
              <div><label className={labelClass}>Observações</label><textarea rows={2} value={form.notes} onChange={e => set("notes", e.target.value)} className={`${inputClass} resize-none`} placeholder="Observações..." /></div>
