...
import { ShoppingBag, Loader2, Plus, Minus, Trash2, MessageCircle, Package } from "lucide-react";

export default function LojaPage() {
  const supabase = createClientComponentClient();
  
  // --- CONFIGURAÇÃO ---
  const ADMIN_PHONE = "5511999999999"; // <--- SEU WHATSAPP AQUI
  ...
  if (loading) return (
    <div className="w-full h-full flex items-center justify-center min-h-[50vh] text-white">
  ...
  return (
    <div className="w-full min-h-screen bg-[#000000] text-white p-4 md:p-8 pb-32 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-2 text-sm">Ferramentas para sua evolução.</p>
      </div>
                ...
                <div className="flex-1 w-full">
                    <div className="flex justify-between items-center mb-2 md:mb-0">
                         ...
                         <button onClick={() => setIsCartOpen(!isCartOpen)} className="text-xs text-[#C9A66B] md:hidden">
                            {isCartOpen ? "Ocultar" : "Ver Itens"}
                         </button>
                    </div>

                    <div className={`${isCartOpen ? 'flex' : 'hidden'} md:flex flex-wrap gap-3 mt-2 md:mt-0`}>
                        {cart.map(item => (
                            <div key={item.id} className="flex items-center gap-2 bg-[#000] px-3 py-1 rounded border border-[#333]">
                                <span className="text-xs font-bold text-white">{item.quantity}x {item.title}</span>
                                <button onClick={() => removeFromCart(item.id)} className="text-red-500 hover:text-red-400"><Trash2 size={12}/></button>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="flex items-center gap-4 w-full md:w-auto border-t md:border-t-0 border-[#333] pt-3 md:pt-0 justify-between md:justify-end">
                    ...
                    <button 
                        ...
                        className="bg-green-600 hover:bg-green-500 text-white px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg shadow-green-900/20"
                    >
