...
import { ArrowLeft, Loader2, Trophy, Download, Send, PlayCircle } from "lucide-react";

// Limpeza de ID garantida
...
export default function AulaPlayerPage() {
  ...
  useEffect(() => { loadData(); }, []);
  useEffect(() => { 
      if (currentLesson) fetchComments(currentLesson.id); 
  }, [currentLesson]);
  ...
  // URL do Iframe segura
  const embedUrl = currentLesson?.video_id 
    ? `https://www.youtube.com/embed/${getCleanVideoId(currentLesson.video_id)}?autoplay=0&rel=0&modestbranding=1` 
    : "";
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-8 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* PLAYER (IFRAME PURO = FUNCIONA SEMPRE) */}
        <div className="lg:col-span-2 space-y-4">
            <div className="aspect-video w-full bg-black rounded-xl overflow-hidden border border-[#333] shadow-2xl relative z-10">
                {embedUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-gray-600">Selecione uma aula</div>
                )}
            </div>
