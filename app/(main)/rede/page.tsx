...
export default function RedePage() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      if (session) {
        ...
        const { data } = await supabase.from("profiles").select("*").eq('invited_by', session.user.id);
  ...
  const inviteLink = `https://mascpro.com/?ref=${userId || ""}`;
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-2 p-1 rounded-xl border border-[#C9A66B]/30 bg-black/40">
          <div className="px-4 py-2">
            <p className="text-[10px] font-bold text-[#C9A66B] uppercase tracking-widest">Link de convite</p>
            <a href={inviteLink} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 font-mono hover:underline">
              {inviteLink}
            </a>
          </div>
          <button onClick={handleCopy} className="bg-[#C9A66B] text-black px-4 py-3 rounded-lg font-bold text-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#0A0A0A] border border-white/5 p-6 rounded-2xl flex items-center gap-5">
          ...
          <div><p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Indicados</p><p className="text-3xl font-black text-white">{indicados.length}</p></div>
        </div>
